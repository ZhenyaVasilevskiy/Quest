  ...
  <div class="finhistory-quest-inner">
    <template v-if="gameStep.isQuestion">
      ...
      <el-row class="q" type="flex">
        ...
        >
          <img src="../../assets/images/finhistory/npc.jpg" class="npc npc--q" />
        ...
        <el-col
          :md="{span: 18}"
          :lg="{span: 7}"
          class="q__block q__block--4"
          v-show="canShowStep(2)"
        >
          <h1 class="q__title">Вопрос</h1>
          <div class="q__text" v-html="gameStep.task"></div>
        </el-col>
        <el-col
          :md="{span: 18}"
          :lg="{span: 8}"
        ...
        >
          ...
          <button
            ...
            :disabled="isDisabledAnswerButton"
            :class="{
                    'button--green': !isDisabledAnswerButton,
                    'button--red': isDisabledAnswerButton
                }"
            @click="setAnswer"
          >
            <span>Ответить</span>
    ...
    <template v-else>
      <div class="finhistory-quest-1" v-if="questionStep == 0">
        ...
        <img src="../../assets/images/finhistory/npc.jpg" class="npc" />
        <p class="finhistory-quest-text" v-html="gameStep.text"></p>
        <p class="finhistory-quest-tip" v-if="gameStep.isQuestion && gameStep.tip">{{gameStep.tip}}</p>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getTip() {
      this.$confirm(``, `Подсказка стоит 5 баллов`, {
        ...
        this.$store.commit("minusPointsCount", 5);
        this.readTip();
      });
    },
    canShowStep(step) {
...
.finhistory-quest-nextButton:hover {
  color: #000;
...
.finhistory-quest-answer .el-checkbox__input.is-checked + .el-checkbox__label {
  color: #000 !important;
}
...
/*Использую что бы определять сейчас мобильная или десктопная версия сайта открыта. Если у блока z-index = 1, то значит мобильная*/
