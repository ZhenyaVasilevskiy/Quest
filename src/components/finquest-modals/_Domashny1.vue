<template>
  <FinquestQuest :gameStep="gameStep">
    <slot #task v-if="curWay == 0">
    ...
    <slot
      #task
      v-else-if="curWay == 1"
    >Перед вами диаграммы доходов и расходов и список недостающих элементов</slot>

    <slot #question v-if="curWay == 0">
    ...
    <slot #question v-else-if="curWay == 1">
    ...
    <slot #question_tip v-if="curWay == 0">Выберите правильные варианты ответов из списка:</slot>
    <slot
      question_tip
      v-else-if="curWay == 1"
    >Перетяните в пропуски недостающие элементы в диаграммы.</slot>
    <slot #question_body v-if="curWay == 0">
      ...
      <div class="finquest-form__select-group">
        <span class="finquest-form__select-group__title">Совокупные ежемесячные расходы семьи:</span>
      ...
      <div class="finquest-form__select-group">
        <span class="finquest-form__select-group__title">Ивановым удастся накопить на ноутбук:</span>
    ...
    <slot #question_body v-else-if="curWay == 1">
...
<script>
import FinquestQuest from "../finquest/FinquestQuest.vue";
export default {
  name: "modal-job-application",
  components: {
    FinquestQuest: FinquestQuest
  },
  data() {
    return {
      step: 1,
  ...
  computed: {
    ...
    gameStep() {
      return this.$store.getters.getFinquest.gameSteps[0];
    }
  },
  watch: {
